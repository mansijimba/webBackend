...
exports.loginUser = async (req, res) => {
  ...
  try {
    ...
    if (
      getUser.isLocked &&
      getUser.lockUntil &&
      new Date() < new Date(getUser.lockUntil)
    ) {
      return res
        .status(403)
        .json({
    ...
    if (!passwordCheck) {
      ...
      if (
        getUser.failedLoginAttempts >= 3 &&
        getUser.failedLoginAttempts < MAX_FAILED_LOGIN
      ) {
        const tempToken = jwt.sign(
          { _id: getUser._id, securityChallenge: true },
          process.env.SECRET,
          { expiresIn: "10m" }
        );
        await getUser.save();
        const questions = (getUser.securityQuestions || []).map((sq) => ({
        ...
        return res
          ...
          .json({
            success: true,
            message: "Security questions required",
  ...
  try {
    ...
    if ((user.failedLoginAttempts || 0) < 3) {
      return res
        ...
        .json({
          success: false,
          message: "Security questions not available yet",
          errors: [],
        });
    }
    ...
    if (user.isLocked) {
      return res
        .status(403)
        .json({ success: false, message: "Account is locked", errors: [] });
    }
    ...
    const tempToken = jwt.sign(
      { _id: user._id, securityChallenge: true },
      process.env.SECRET,
      { expiresIn: "10m" }
    );
...
exports.requestUnlock = async (req, res) => {
  try {
    ...
    if (!user.isLocked)
      return res
        .status(400)
        .json({ success: false, message: "Account is not locked", errors: [] });
    ...
    return res
      .status(200)
      .json({ success: true, unlockLink: frontendUnlockLink });
  } catch (err) {
  ...
  try {
    ...
    if (new Date() > new Date(user.unlockTokenExpiry)) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Unlock token has expired. Request a new one.",
...
exports.setupMfa = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        success: true,
        message: "MFA temp secret generated",
...
exports.verifyMfaSetup = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "MFA enabled", errors: [] });
  } catch (err) {
...
exports.verifyMfaLogin = async (req, res) => {
  try {
    ...
    const finalToken = jwt.sign(payload, process.env.SECRET, {
      expiresIn: "7d",
    });
  ...
  try {
    ...
    for (const provided of answers) {
      const stored = user.securityQuestions.find(
        (sq) => sq.question === provided.question
      );
      if (!stored) {
        allMatch = false;
        break;
    ...
    if (!allMatch) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Security answers incorrect",
          errors: [],
        });
    }
